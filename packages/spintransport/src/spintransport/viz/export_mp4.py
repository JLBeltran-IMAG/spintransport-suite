...
#
# Module: spintransport.viz.export_mp4
# Brief : Render |psi|^2 time evolution to MP4 (ffmpeg) with layout options.
# Project: spintransport-suite
# Authors: Jorge Luis Beltran Diaz and Leovildo Diago Cisneros
"""
-----------------------------------------------------------------------------------------------
| MP4 exporter using Matplotlib + ffmpeg; handles stride, fps, energy selection, and styling. |
-----------------------------------------------------------------------------------------------

Export time evolution of |psi(y,t;E)|^2 to an MP4 video.
...
- Uses Matplotlib + FFMpegWriter (requires ffmpeg in PATH).
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(description="Export time evolution of |psi|^2 to MP4.")
    ...
    p.add_argument("--fps", type=int, default=24, help="Frames per second (video).")
    p.add_argument("--stride", type=int, default=1, help="Use every STRIDE-th frame (>=1).")
    p.add_argument("--dpi", type=int, default=140, help="Figure DPI for the video.")
    p.add_argument("--style", default="default", help="Matplotlib style (e.g., 'seaborn-v0_8-paper').")
    p.add_argument("--outfile", type=Path, default=None, help="Output MP4 filename. Default auto in --dir.")
    p.add_argument("--codec", default="libx264", help="FFmpeg codec (default: libx264).")
    p.add_argument("--bitrate", type=int, default=2000, help="FFmpeg bitrate in kbps (e.g., 2000).")
...
def main():
    ...
    # Validate ffmpeg presence early
    if not animation.writers.is_available("ffmpeg"):
        raise RuntimeError(
            "ffmpeg writer not available. Please install FFmpeg (e.g., `sudo apt install ffmpeg`) and retry."
        )
    ...
    writer = animation.FFMpegWriter(
        ...
        extra_args=["-pix_fmt", "yuv420p"],  # improve compatibility
    )

    tag = f"{args.layout}"
    outfile = args.outfile or (sim.base_dir / f"evolution_E{E}_{tag}.mp4")
    outfile.parent.mkdir(parents=True, exist_ok=True)

    print(f"[i] Writing MP4 to: {outfile}")
